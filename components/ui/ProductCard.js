        ...
        <div className={`flex flex-col ${isHomePage ? 'w-[280px] sm:w-[350px] lg:w-[370px] h-[450px] sm:h-[500px] lg:h-[550px] max-md:w-[310px]' : 'w-[160px] sm:w-[220px] lg:w-[250px] h-[290px] sm:h-[320px] lg:h-[350px]'} mx-auto `}>
                ...
                <div className="w-full flex justify-between">
                    {isNew && (
                        <div className={`z-10 ${isHomePage ? 'w-28 h-7' : 'w-fit h-fit sm:px-2'} rounded-md bg-black max-md:w-fit max-md:h-fit max-md:px-2 max-md:rounded-sm`}>
                            ...
                            <p className={`text-white ${isHomePage ? 'text-lg' : 'text-base max-md:text-[11px]'}  text-center`}>Nouveau</p>
                        </div>
                    )}
                    {!isNew && <div></div>}
                    ...
                    <div className={`z-10 ${isHomePage ? 'w-28 h-7 ' : 'w-fit h-fit sm:px-2 '} rounded-md ${getTypeColor(ProductType)} max-md:w-fit max-md:h-fit max-md:px-2 max-md:rounded-sm`}>
                        ...
                        <p className={`text-white ${isHomePage ? 'text-lg' : 'text-base max-md:text-[11px]'}  text-center`}>{ProductType}</p>
                    </div>
                </div>
            ...
            <div className={`w-full ${isHomePage ? 'h-1/9' : 'h-1/9 max-md:h-2/9 max-md:flex-col max-md:justify-center'}  border-2 flex justify-between items-center p-2`}>
               <div className="flex items-center gap-2">
                    {/* fragance name text */}
                    <p className={`text-black ${isHomePage ? 'text-xl' : 'text-sm max-md:text-[13px]'}  font-bold max-md:text-base`}>{productName}</p>
                    {isHomePage && 
                        <div className="rounded-full w-2 h-2 bg-black"></div>
                    }
                    {/* fragance volume text */}
                    <p className={`${isHomePage ? 'text-xl' : 'text-sm max-md:text-[13px]'} max-md:text-base`}>{ProductVolume}</p>
               </div>

               <div className="flex items-center">
                {/* price text */}
                    <p className={`${isHomePage ? 'text-xl' : 'text-sm max-md:text-[12px]'} font-bold max-md:text-base`}>{productPrice}</p>
